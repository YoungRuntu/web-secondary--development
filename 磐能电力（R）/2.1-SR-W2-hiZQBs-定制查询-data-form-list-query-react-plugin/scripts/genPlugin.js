...
fs.readdirSync(path.resolve(__dirname, "../build/static/js")).map((file) => {
  if (path.extname(file) === ".js") {
    if (file.indexOf("main") > -1) {
      mainFileName = file
    }
    fs.copyFileSync(path.resolve(__dirname, `../build/static/js/${file}`), path.resolve(__dirname, `../pluginTemp/js/${file}`))
  } else {
...
// copy 静态文件
if (fs.existsSync(path.resolve(__dirname, "../build/static/media"))) {
  fs.mkdirSync(path.resolve(__dirname, "../pluginTemp/media"))
  fs.readdirSync(path.resolve(__dirname, "../build/static/media")).map((file) => {
    fs.copyFileSync(path.resolve(__dirname, `../build/static/media/${file}`), path.resolve(__dirname, `../pluginTemp/media/${file}`))
  })
}
